...
describe(`Модуль generate`, () => {
  ...
  describe(`Правила для поля "date"`, () => {
    it(`Существует поле "date" типа "number" от сейчас минус ${conditions.date.daysBack} дней в формате UNIX`, () => {
      ...
      assert(entity[`date`] >= (new Date().setUTCHours(0, 0, 0, 0) / 1000 - conditions.date.dayUNIX * conditions.date.daysBack), `Значение поля "date" меньше чем ${conditions.date.daysBack} дней назад`);
      assert(entity[`date`] <= (new Date().setUTCHours(23, 59, 59, 0) / 1000), `Значение поля "date" больше чем сейчас`);
    });
  });
});
